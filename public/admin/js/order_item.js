			...
			function getOrder_items(searchObj) {
				...
				updateOrder_itemsTableHeaderSort();
				  ...
				  $.ajax({
					...
					<td>${ result[i] ? result[i].returned ? result[i].returned : ' '  : ' '  }</td><td><span class="btn btn-link btn-sm" onclick="editOrder_itemModal(  '${result[i].order_id}', '${result[i].drug_code}', '${result[i].quantity}', '${result[i].returned}', ${result[i].id} )">Edit</span><span class="btn btn-link btn-sm" onclick="deleteOrder_itemModal(${result[i].id})">Delete</span></td>
				          </tr>
				        `);
				      }
				...
				function addOrder_item() {
				  $.ajax({
				    ...
				    method: "POST",
				    data: {
				    	order_id :  $("#addOrderItemOrderIdInput").val() ,drug_code :  $("#addOrderItemDrugCodeInput").val() ,quantity :  $("#addOrderItemQuantityInput").val() ,returned :  $("#addOrderItemReturnedInput").val() ,
				      token: Cookies.get("token"),
				    },
				...
				function updateOrder_item()  {
				  $.ajax({
				    ...
				    method: "POST",
				    data: {
				    	order_id : $("#editOrderItemOrderIdInput").val(),drug_code : $("#editOrderItemDrugCodeInput").val(),quantity : $("#editOrderItemQuantityInput").val(),returned : $("#editOrderItemReturnedInput").val(),id : $("#editOrderItemOrder_itemId").val(),				    
				      token: Cookies.get("token"),
				    },
				    success: function (result) {
				      console.log(result);
				...
				function editOrder_itemModal(order_id,drug_code,quantity,returned,id) {
				  ...
				  $("#editOrderItemOrderIdInput").val(order_id);$("#editOrderItemDrugCodeInput").val(drug_code);$("#editOrderItemQuantityInput").val(quantity);$("#editOrderItemReturnedInput").val(returned);
				}
