...
function updatePurchase_return_drugsTableHeaderSort() {
  ...
  $(
    "#" +
      Purchase_return_drugTableOrderField +
      "Sort" +
      Purchase_return_drugTableOrderFieldBy
  ).removeClass("fade-l");
}
...
function getPurchase_return_drugs(searchObj) {
  ...
  $.ajax({
    url: "/pharmacies/getpurchase_return_drugs",
    method: "POST",
    data: data,
    success: function (resultData) {
      ...
      for (var i = 0; i < result.length; i++) {
        $("#purchase_return_drugTableBody").append(`
				          <tr>
				          	<td> <input onclick="checkSelected('Purchase_return_drug')" type="checkbox" class=" checkbox-Purchase_return_drug "  data-id="${
                      result[i].id
                    }" /> </td>
					<td>${result[i] ? (result[i].id ? result[i].id : " ") : " "}</td>
					<td>${
            result[i]
              ? result[i].purchase_drug_item_id
                ? result[i].purchase_drug_item_id
                : " "
              : " "
          }</td>
					...
					<td>${
            result[i]
              ? result[i].return_date
                ? result[i].return_date
                : " "
              : " "
          }</td><td><span class="btn btn-link btn-sm" onclick="editPurchase_return_drugModal(  '${
          result[i].purchase_drug_item_id
        }', '${result[i].quantity}', '${result[i].return_date}', ${
          result[i].id
        } )">Edit</span><span class="btn btn-link btn-sm" onclick="deletePurchase_return_drugModal(${
          result[i].id
        })">Delete</span></td>
				          </tr>
				        `);
      ...
      getPaginate(
        count,
        changePurchase_return_drugsTableOffset,
        Purchase_return_drugTableLimit,
        Purchase_return_drugTableOffset,
        "Purchase_return_drug"
      );
    },
  });
}
...
$("#searchPurchase_return_drugForm").on("submit", (ev) => {
  ...
  tempForm = ev;
  var searchObj = {};
  $("#searchPurchase_return_drugForm")
    .serializeArray()
    .map((i) => {
...
function addPurchase_return_drug() {
  $.ajax({
    url: "/pharmacies/addpurchase_return_drug",
    method: "POST",
    data: {
      purchase_drug_item_id: $(
        "#addPurchaseReturnDrugPurchaseDrugItemIdInput"
      ).val(),
      quantity: $("#addPurchaseReturnDrugQuantityInput").val(),
      return_date: $("#addPurchaseReturnDrugReturnDateInput").val(),
      token: Cookies.get("token"),
    },
    success: function (result) {
      ...
      if (result.status == "success") {
        $(
          "#addPurchase_return_drugForm input, #addPurchase_return_drugForm textarea"
        ).val("");
...
function updatePurchase_return_drug() {
  $.ajax({
    url: "/pharmacies/updatepurchase_return_drug",
    method: "POST",
    data: {
      purchase_drug_item_id: $(
        "#editPurchaseReturnDrugPurchaseDrugItemIdInput"
      ).val(),
      quantity: $("#editPurchaseReturnDrugQuantityInput").val(),
      return_date: $("#editPurchaseReturnDrugReturnDateInput").val(),
      id: $("#editPurchaseReturnDrugPurchase_return_drugId").val(),
      token: Cookies.get("token"),
    },
    success: function (result) {
      console.log(result);
      ...
      if (result.status == "success") {
        $(
          "#editPurchase_return_drugForm input, #editPurchase_return_drugForm textarea"
        ).val("");
...
function editPurchase_return_drugModal(
  purchase_drug_item_id,
  quantity,
  return_date,
  id
  ...
  $("#editPurchaseReturnDrugPurchaseDrugItemIdInput").val(
    purchase_drug_item_id
  );
  $("#editPurchaseReturnDrugQuantityInput").val(quantity);
  $("#editPurchaseReturnDrugReturnDateInput").val(return_date);
}
...
function deletePurchase_return_drug(id) {
  $.ajax({
    url: "/pharmacies/deletepurchase_return_drug",
    method: "POST",
    data: {
      id: id,
      token: Cookies.get("token"),
...
function bulkDeletePurchase_return_drug(ids) {
  $.ajax({
    url: "/pharmacies/bulkdeletepurchase_return_drug",
    method: "POST",
    data: {
      ids: ids,
      token: Cookies.get("token"),
