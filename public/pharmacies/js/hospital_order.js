          ...
          <td>${
            ...
            result[i]
              ? result[i].status != null
                ...
                : " "
              : " "
          } <span class="btn btn-sm btn-warning" onclick="showOrderStatusEditDialog('${
          result[i].status
        }',${result[i].id})">Edit</span> </td><td>
...
function showOrderStatusEditDialog(currentStatus, order_id) {
  $("#orderStatusId").val(order_id);
  $("#orderStatusModal").modal("show");
  switch (parseInt(currentStatus)) {
    case 1:
      $("#orderCurrentStatus").text("Delivered");
      $("#orderNewStatus").val(1);
      break;
    case 2:
      $("#orderCurrentStatus").text("Cancelled");
      $("#orderNewStatus").val(2);
      break;
    case 3:
      $("#orderCurrentStatus").text("Requested");
      $("#orderNewStatus").val(3);
      break;
    case 4:
      $("#orderCurrentStatus").text("Accepted");
      $("#orderNewStatus").val(4);
      break;
    case 5:
      $("#orderCurrentStatus").text("Preparing");
      $("#orderNewStatus").val(5);
      break;
    case 6:
      $("#orderCurrentStatus").text("Shipped");
      $("#orderNewStatus").val(6);
      break;
    case 7:
      $("#orderCurrentStatus").text("Fulfilled");
      $("#orderNewStatus").val(7);
      break;
    default:
...
async function updateOrderStatus() {
  const data = await fetch("/pharmacies/updateorderstatus", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      order_id: $("#orderStatusId").val(),
      status: $("#orderNewStatus").val(),
      message: $("#orderStatusMessage").val(),
      token: Cookies.get("token"),
    }),
  })
    .then((response) => response.json())
