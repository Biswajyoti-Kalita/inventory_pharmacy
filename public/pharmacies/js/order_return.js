...
function updateOrder_returnsTableHeaderSort() {
  ...
  $(
    "#" + Order_returnTableOrderField + "Sort" + Order_returnTableOrderFieldBy
  ).removeClass("fade-l");
}
...
function getOrder_returns(searchObj) {
  ...
  $.ajax({
    url: "/pharmacies/getorder_returns",
    method: "POST",
    data: data,
    success: function (resultData) {
      ...
      for (var i = 0; i < result.length; i++) {
        $("#order_returnTableBody").append(`
				          <tr>
				          	<td> <input onclick="checkSelected('Order_return')" type="checkbox" class=" checkbox-Order_return "  data-id="${
                      result[i].id
                    }" /> </td>
					<td>${result[i] ? (result[i].id ? result[i].id : " ") : " "}</td>
					<td>${
            result[i]
              ? result[i].order_item_id
                ? result[i].order_item_id
                : " "
              : " "
          }</td>
					<td>${
            result[i] ? (result[i].quantity ? result[i].quantity : " ") : " "
          }</td><td><span class="btn btn-link btn-sm" onclick="editOrder_returnModal(  '${
          result[i].order_item_id
        }', '${result[i].quantity}', ${
          result[i].id
        } )">Edit</span><span class="btn btn-link btn-sm" onclick="deleteOrder_returnModal(${
          result[i].id
        })">Delete</span></td>
				          </tr>
				        `);
      ...
      getPaginate(
        count,
        changeOrder_returnsTableOffset,
        Order_returnTableLimit,
        Order_returnTableOffset,
        "Order_return"
      );
    },
  });
...
function addOrder_return() {
  $.ajax({
    url: "/pharmacies/addorder_return",
    method: "POST",
    data: {
      order_item_id: $("#addOrderReturnOrderItemIdInput").val(),
      quantity: $("#addOrderReturnQuantityInput").val(),
      token: Cookies.get("token"),
    },
...
function updateOrder_return() {
  $.ajax({
    url: "/pharmacies/updateorder_return",
    method: "POST",
    data: {
      order_item_id: $("#editOrderReturnOrderItemIdInput").val(),
      quantity: $("#editOrderReturnQuantityInput").val(),
      id: $("#editOrderReturnOrder_returnId").val(),
      token: Cookies.get("token"),
    },
    success: function (result) {
      console.log(result);
...
function editOrder_returnModal(order_item_id, quantity, id) {
  ...
  $("#editOrderReturnOrderItemIdInput").val(order_item_id);
  $("#editOrderReturnQuantityInput").val(quantity);
}
...
function deleteOrder_return(id) {
  $.ajax({
    url: "/pharmacies/deleteorder_return",
    method: "POST",
    data: {
      id: id,
      token: Cookies.get("token"),
...
function bulkDeleteOrder_return(ids) {
  $.ajax({
    url: "/pharmacies/bulkdeleteorder_return",
    method: "POST",
    data: {
      ids: ids,
      token: Cookies.get("token"),
