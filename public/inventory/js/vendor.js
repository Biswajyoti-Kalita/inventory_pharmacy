...
function updateVendorsTableHeaderSort() {
  ...
  $("#" + VendorTableOrderField + "Sort" + VendorTableOrderFieldBy).removeClass(
    "fade-l"
  );
}
    ...
    success: function (resultData) {
      ...
      for (var i = 0; i < result.length; i++) {
        ...
        $("#vendorTableBody").append(`
				          <tr>
				          	<td> <input onclick="checkSelected('Vendor')" type="checkbox" class=" checkbox-Vendor "  data-id="${
                      result[i].id
                    }" /> </td>
            ...
            result[i]
              ...
              : " "
					...
					<td>${
            result[i] ? (result[i].zip ? result[i].zip : " ") : " "
          }</td><td><span class="btn btn-link btn-sm" onclick="editVendorModal(  '${
          result[i].name
        }', '${result[i].vendor_code}', '${result[i].street_address_1}', '${
          result[i].street_address_2
        }', '${result[i].city}', '${result[i].state}', '${
          result[i].country
        }', '${result[i].zip}', ${
          result[i].id
        } )">Edit</span><span class="btn btn-link btn-sm" onclick="deleteVendorModal(${
          result[i].id
        })">Delete</span></td>
				          </tr>
				        `);
...
function addVendor() {
  $.ajax({
    url: "/pharmacies/addvendor",
    method: "POST",
    data: {
      name: $("#addVendorNameInput").val(),
      vendor_code: $("#addVendorVendorCodeInput").val(),
      street_address_1: $("#addVendorStreetAddress1Input").val(),
      street_address_2: $("#addVendorStreetAddress2Input").val(),
      city: $("#addVendorCityInput").val(),
      state: $("#addVendorStateInput").val(),
      country: $("#addVendorCountryInput").val(),
      zip: $("#addVendorZipInput").val(),
      token: Cookies.get("token"),
    },
...
function updateVendor() {
  $.ajax({
    url: "/pharmacies/updatevendor",
    method: "POST",
    data: {
      name: $("#editVendorNameInput").val(),
      vendor_code: $("#editVendorVendorCodeInput").val(),
      street_address_1: $("#editVendorStreetAddress1Input").val(),
      street_address_2: $("#editVendorStreetAddress2Input").val(),
      city: $("#editVendorCityInput").val(),
      state: $("#editVendorStateInput").val(),
      country: $("#editVendorCountryInput").val(),
      zip: $("#editVendorZipInput").val(),
      id: $("#editVendorVendorId").val(),
      token: Cookies.get("token"),
    },
    success: function (result) {
      console.log(result);
...
function editVendorModal(
  ...
  $("#editVendorNameInput").val(name);
  $("#editVendorVendorCodeInput").val(vendor_code);
  $("#editVendorStreetAddress1Input").val(street_address_1);
  $("#editVendorStreetAddress2Input").val(street_address_2);
  $("#editVendorCityInput").val(city);
  $("#editVendorStateInput").val(state);
  $("#editVendorCountryInput").val(country);
  $("#editVendorZipInput").val(zip);
}
...
function bulkDeleteVendor(ids) {
  $.ajax({
    url: "/pharmacies/bulkdeletevendor",
    method: "POST",
    data: {
      ids: ids,
      token: Cookies.get("token"),
