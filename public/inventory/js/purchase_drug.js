...
function updatePurchase_drugsTableHeaderSort() {
  ...
  $(
    "#" + Purchase_drugTableOrderField + "Sort" + Purchase_drugTableOrderFieldBy
  ).removeClass("fade-l");
}
    ...
    success: function (resultData) {
      ...
      for (var i = 0; i < result.length; i++) {
        $("#purchase_drugTableBody").append(`
				          <tr>
				          	<td> <input onclick="checkSelected('Purchase_drug')" type="checkbox" class=" checkbox-Purchase_drug "  data-id="${
                      result[i].id
                    }" /> </td>
					...
					<td>${
            result[i]
              ? result[i].purchase_date
                ? result[i].purchase_date
                : " "
              : " "
          }</td><td><span class="btn btn-link btn-sm" onclick="editPurchase_drugModal(  '${
          result[i].bill_id
        }', '${result[i].vendor_id}', '${result[i].purchase_date}', ${
          result[i].id
        } )">Edit</span><span class="btn btn-link btn-sm" onclick="deletePurchase_drugModal(${
          result[i].id
        })">Delete</span></td>
				          </tr>
				        `);
      ...
      getPaginate(
        count,
        changePurchase_drugsTableOffset,
        Purchase_drugTableLimit,
        Purchase_drugTableOffset,
        "Purchase_drug"
      );
    },
  });
...
function addPurchase_drug() {
  $.ajax({
    ...
    method: "POST",
    data: {
      bill_id: $("#addPurchaseDrugBillIdInput").val(),
      vendor_id: $("#addPurchaseDrugVendorIdInput").val(),
      purchase_date: $("#addPurchaseDrugPurchaseDateInput").val(),
      token: Cookies.get("token"),
    },
...
function updatePurchase_drug() {
  $.ajax({
    ...
    method: "POST",
    data: {
      bill_id: $("#editPurchaseDrugBillIdInput").val(),
      vendor_id: $("#editPurchaseDrugVendorIdInput").val(),
      purchase_date: $("#editPurchaseDrugPurchaseDateInput").val(),
      id: $("#editPurchaseDrugPurchase_drugId").val(),
      token: Cookies.get("token"),
    },
    success: function (result) {
      console.log(result);
...
function editPurchase_drugModal(bill_id, vendor_id, purchase_date, id) {
  ...
  $("#editPurchaseDrugBillIdInput").val(bill_id);
  $("#editPurchaseDrugVendorIdInput").val(vendor_id);
  $("#editPurchaseDrugPurchaseDateInput").val(purchase_date);
}
