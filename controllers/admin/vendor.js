...
const role = 0;
					app.post("/admin/addvendor",roleService.verifyRole(role),   async function(req, res) {
					    try {
							  ...
							  if(req.body.vendor_code === null || req.body.vendor_code === undefined)
								  return res.send({status  : "error", message : " Vendor Code is required " })
			...
			zip : req.body.zip,
					...
					app.post("/admin/updatevendor",  roleService.verifyRole(role),  async function(req, res) {
					    try {
			...
			zip : req.body.zip,
					...
					app.post("/admin/deletevendor",  roleService.verifyRole(role),  async function(req, res) {
					...
					app.post("/admin/bulkdeletevendor", roleService.verifyRole(role),  async function(req, res) {
				...
				app.post("/admin/getvendors",  roleService.verifyRole(role),  async function(req, res) {
	...
	;
				...
				app.post("/admin/getvendor",  roleService.verifyRole(role),  async function(req, res) {
				    try {
				        const vendor = await db.vendor.findOne({
				        	...
				        	attributes: ['id','name','vendor_code','street_address_1','street_address_2','city','state','country','zip']
				        });
				        res.send(vendor);
				    } catch (error) {
				        console.log(error);
